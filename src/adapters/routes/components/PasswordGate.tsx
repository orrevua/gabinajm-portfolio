...
export const PasswordGate: React.FC<PasswordGateProps> = ({ slug, projectTitle }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(data.error || "The password is incorrect.");
    } catch {
      ...
      <div className="flex-1 flex items-start justify-center px-6">
        ...
        >
          ...
          <form onSubmit={handleSubmit}>
            <label htmlFor="project-password" className="block text-sm font-semibold text-foreground mb-2">
              Password
            </label>
            ...
            {error && (
              <p className="text-sm text-foreground mt-1.5 mb-4" role="alert">
                {error}
              </p>
            )}
