  ...
  return (
    <div className="flex items-center gap-2 bg-white rounded-pill px-4 py-2 shadow-[0_8px_16px_-10px_rgba(0,0,0,0.25)]">
      ...
      <div className="relative grid grid-cols-2">
        <div
          className="absolute top-0 bottom-0 w-1/2 rounded-pill bg-gradient-to-r from-accent to-accent-purple transition-transform duration-300 ease-in-out"
        ...
        {(["en", "pt"] as Locale[]).map((l) => (
          <button
            key={l}
            onClick={() => setLocale(l)}
            className={`relative z-10 px-3 py-1 rounded-pill text-sm text-center transition-colors duration-300 ${
              locale === l
                ? "text-white font-bold"
                : "text-[#0A0A0A]/60 font-medium hover:text-[#0A0A0A] hover:bg-[#E5E7EB]"
            }`}
